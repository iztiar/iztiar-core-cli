...
import { Iztiar, coreCmdline, coreError, coreLogger, msg, utils } from './imports.js';
...
const st = {
    ...
    app: {
        consoleLevel: Iztiar.c.verbose.NORMAL,
...
function _fillupAppConfig( json ){
    ...
    // console level
    if( !json.consoleLevel || _opts.verbose !== coreConfig.getDefaultVerbose()){
        _filled.consoleLevel = _opts.verbose;
    }
    // log level
    ...
    // stop port numner
    if( !Object.keys( json ).includes( 'stop' )){
        json.stop = {};
        _filled.stop = {};
    }
    if( !json.stop.port || _opts.stopPort !== coreConfig.getDefaultAppStopPort()){
        _filled.stop.port = _opts.stopPort;
    }
    // storage dir
...
// overriding it with command-line options (if apply)
//  param {JSON} json take the read controller configuration (if any)
//  param {string} name name of the service controller
//  returns {JSON} the completed json
function _fillupControllerConfig( json, name ){
    ...
    if( !Object.keys( json ).includes( 'managed' )){
        json.managed = [];
        _filled.managed = [];
    }
    // manager name
    ...
    //console.log( 'name='+name, 'opts.name='+_opts.name, 'manager='+_opts.manager );
    if( _opts.name === name && _opts.manager !== coreConfig.getDefaultManagerName()){
        _filled.manager = _opts.manager;
    }
    if( _filled.manager === coreConfig.getDefaultManagerName()){
        _filled.manager='';
    }
    // check that we don't have both managed and manager (#29)
    if( _filled.managed.lenth && _filled.manager.length ){
        msg.error( CONFIG_BOTHMANAGEDANDMANAGER );
        throw new coreError( CONFIG_BOTHMANAGEDANDMANAGER );
    }
    //console.log( _filled );
    return _filled;
}
...
export class coreConfig {
    ...
    static getControllerFilledConfig( name ){
        ...
        if( !_filled ){
            _configs[name] = _fillupControllerConfig( coreConfig.getControllerRawConfig( name ), name );
    ...
    /**
     * @returns {integer} the port number available for the cli
     */
    static getDefaultAppStopPort(){
        return st.app.stop.port;
    }
    ...
    /**
     * @returns {integre} the default verbosity level
     */
    static getDefaultVerbose(){
        return st.app.consoleLevel;
    }
