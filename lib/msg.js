...
export class msg {
    ...
    static info(){
        _log( 'info', 'info', chalk.cyan, ...arguments );
    }

    static verbose(){
      _log( 'info', 'verbose', null, ...arguments );
    }

    static debug(){
      _log( 'debug', 'debug', chalk.white, '(debug)', ...arguments );
    }
    ...
    static consoleLevel( level ){
        if( utils.isInt( level )){
            _singleton._consoleLevel = level;
        }
        return _singleton._consoleLevel;
    }
