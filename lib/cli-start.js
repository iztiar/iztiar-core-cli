...
import { Iztiar, coreBroker, coreConfig, coreController, coreError, coreForkable, coreLogger } from './imports.js';
...
export function cliStart( serviceName, options={} ){
    ...
    coreLogger.debug( 'cliStart()', 'processName='+_processName, 'serviceName='+serviceName, 'options', options, 'sceConfig=', _sceConfig );
    ...
    if( !_processName ){
        ...
        coreLogger.debug( 'cliStart().ipcTarget='+_ipcTarget );
                        ...
                        .then(( _local ) => {
                            if( expected ){
                                ...
                                if( _local.reasons.length === 0 ){
                                    ...
                                    console.log( chalk.yellow( 'Service is said running, but exhibits', _local.reasons.length,'error message(s), is not startable' ));
        ...
        const _startPromise = function( res, name ){
            if( res.startable ){
                return new Promise(( resolve, reject ) => {
                    ...
                    console.log( ' - (main) requesting for '+serviceName+' coreController to start...' );
        ...
        // a boolean function to be used in _racingPromise()
        const _testTarget = function(){
            //coreLogger.debug( 'cliStart().testTarget returns '+( _ipcCount === _ipcTarget ));
        ...
        const _waitFor = function( res, boolFn, parmsFn, timeout ){
            coreLogger.debug( 'cliStart()._waitFor() timeout='+timeout );
            let _end = Date.now()+timeout;
            return new Promise(( resolve, reject ) => {
                const _id = setInterval(() => {
                    if( boolFn( parmsFn )){
                        clearInterval( _id );
                        coreLogger.debug( 'cliStart()._waitFor() condition is met' );
                        res.racing = true;
                        resolve( res );
                    }
                    if( Date.now() > _end ){
                        clearInterval( _id );
                        coreLogger.debug( 'cliStart()._waitFor() timeout is reached' );
                        console.log( chalk.yellow( 'timeout reached while waiting for startup...' ));
                        res.racing = false;
                        resolve( res );
                    }
                }, 10 );
