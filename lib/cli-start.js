...
export function cliStart( name, cbExit ){
    ...
    if( !process.env[coreForkable.id] ){
        coreForkable.fork( coreForkable.c.FORKABLE_CONTROLLER, cbExit );
    ...
    // if we are in a forked process dedicated to this task, then go
        ...
        if( _config.broker.enabled ){
            controller.registerBroker( coreForkable.fork( coreForkable.c.FORKABLE_BROKER, null, coreController.OnMessage, controller ));
        }
