...
export function coreStart( name, cb ){
    ...
    if( !process.env[coreForkable.id] ){
        coreForkable.fork( coreForkable.act.FORKABLE_CONTROLLER, cb );

    // if we are in a forked process dedicated to this task, then go
    } else if( process.env[coreForkable.id] === coreForkable.act.FORKABLE_CONTROLLER ){
        ...
        if( _config.broker.enabled ){
            controller.registerBroker( coreForkable.fork( coreForkable.act.FORKABLE_BROKER ));
        }

    // we are in a forked process dedicated to the broker, then go
    } else if( process.env[coreForkable.id] === coreForkable.act.FORKABLE_BROKER ){
