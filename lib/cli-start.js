...
export function cliStart( config, cbExit ){

    if( !Iztiar.envForked()){
        console.log( 'Starting '+config.controller.name+' service(s)' );
    }

    let _procName = Iztiar.envForked();
    coreLogger.debug( 'coreStart() procName='+_procName, 'config=', config );
    ...
    const onStartup = function( child, messageData, parms ){
        ...
        if( _forkable === Iztiar.c.forkable.CONTROLLER ){
            _msg = '(main) coreController ' + messageData[_forkable].config.controller.name;
            _msg += ' successfully startup, listening on port ' + messageData[_forkable].listening;
        } else {
            _msg = '(coreController) coreBroker successfully startup, managed by '+messageData[_forkable].config.controller.name+' coreController';
        ...
        console.log( ' + '+_msg );
        if( forker.ipcCount == forker.ipcTarget ){
            console.log( chalk.green( 'Service(s) '+config.controller.name+' successfully started.' ));
        }
    }
    ...
    if( !_procName ){
        forker = coreController.getForker( config );
    ...
    // if we are in a forked process dedicated to this task, then go
        ...
        controller = new coreController( config );
        controller.start();
        ...
        if( config.broker.enabled ){
            forker = coreBroker.getForker( config );
