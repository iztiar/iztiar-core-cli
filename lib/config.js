...
export class coreConfig {
    ...
    static GetDefaultBrokerEnabled(){
        let _def = coreConfig.s.controller.broker.enabled;
        if( _def === Iztiar.const.default ){
            _def = true;
            coreConfig.s.controller.broker.enabled = _def;
        }
        return _def;
    }
    ...
    _fillupControllerConfig( config, name ){
        ...
        config.manager.enabled = ( 
            ...
            config.manager.name !== Iztiar.const.default && 
            config.manager.name !== Iztiar.const.none );
        // broker enabled
    ...
    _readJson( name, def ){
        const _path = path.join( Iztiar.rt.storageDir, coreConfig.c.confDir, name+'.json' );
    ...
    getControllerRuntimeName( name ){
        return  this.getControllerRuntimePrefix() + ( name || Iztiar.const.default );
    }

    getControllerRuntimePrefix(){
        return 'controller-';
    }
    ...
    getPidDir(){
        return path.join( Iztiar.rt.storageDir, coreConfig.c.pidDir );
    }
}
