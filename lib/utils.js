...
export const utils = {
     ...
     */
    jsonReadFileSync: function( path ){
        coreLogger.debug( 'utils.jsonReadFileSync() '+path );
        let _json = null;
        try {
            _json = JSON.parse( fs.readFileSync( path, { encoding: 'utf8' }));
        } catch( e ){
            if( e.code !== 'ENOENT' ){
                throw new coreResult( e );
            }
            coreLogger.warn( 'utils.jsonReadFileSync() '+path+': file not found or not readable' );
            _json = null;
        }
        return _json;
    },
