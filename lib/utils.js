...
export const utils = {
     ...
     isAlivePid: function( pid ){
        ...
        return new Promise(( resolve, reject ) => {
            ps({ pid:pid, fields:[ 'pid','user','time','etime' ]})
                .then(( res ) => {
                    coreLogger.debug( 'utils.isAlivePid()', 'pid='+pid, 'resolved with res', res );
                    resolve( res.length === 1 ? res : false );
                }, ( rej ) => {
                    coreLogger.debug( 'utils.isAlivePid()', 'pid='+pid, 'rejected, resolving falsy' );
                    resolve( false );
                })
                .catch(( e ) => {
                    coreLogger.error( 'utils.isAlivePid()', 'pid='+pid, 'resolving falsy', e.name, e.message );
                    resolve( false );
                });
        });
    },
    ...
    isAlivePort: function( port ){
        ...
        return new Promise(( resolve, reject ) => {
            utils.tcpRequest( port, 'iz.ping' )
                .then(( res ) => {
                    coreLogger.debug( 'utils.isAlivePort()', 'port='+port, 'resolved with res', res );
                    resolve( res );
                }, ( rej ) => {
                    coreLogger.debug( 'utils.isAlivePort()', 'port='+port, 'rejected, resolving falsy' );
                    resolve( false );
                })
                .catch(( e ) => {
                    coreLogger.error( 'utils.isAlivePort()', 'port='+port, 'resolving falsy', e.name, e.message );
                    resolve( false );
                });
        });
    },
    ...
    jsonRemoveKeySync: function( fname, key, deleteEmpty=true ){
        coreLogger.debug( 'utils.jsonRemoveKeySync()', 'fname='+fname, 'key='+key, 'deleteEmpty='+deleteEmpty );
    ...
    jsonWriteFileSync: function( fname, obj, orig ){
        ...
        if( e ){
            coreLogger.error( 'utils.jsonWriteFileSync().makeFnameDirExists()', e.name, e.message );
            throw new coreError( e );
        }
        ...
        try {
            ...
            coreLogger.error( 'utils.jsonWriteFileSync().writeFileSync()', e.name, e.message );
            throw new coreError( e );
        }
        return obj;
    },
        ...
        return new Promise(( resolve, reject ) => {
            try {
                ...
                client.on( 'data', ( data ) => {
                    ...
                    client.end();
                    coreLogger.debug( 'utils.tcpRequest() resolves with', _json );
                    resolve( _json );
                });
                client.on( 'error', ( e ) => {
                ...
                coreLogger.error( 'utils.tcpRequest().catch()', e.name, e.message );
                reject( e.message );
            }
        });
    },
    ...
    unlink: function( fname ){
        try {
            ...
            coreLogger.error( 'utils.unlink().catch()', e.name, e.message );
            throw new coreError( e );
        }
    },

    /**
     * @param {Promise} promise a Promise, which eventually resolves to true
     * @param {integer} timeout the timeout (ms) to be waited for the promiseFn be resolved
     * @returns {Promise} which will eventually resolve as true if PromiseFn has resolved, or false if timeout (ms) has been reached
     */
    waitFor: function( promise, timeout ){
        return Promise.race([ promise, utils.waitForTimeoutPromise( timeout )]);
    },

    /**
     * @param {integer} timeout the timeout (ms) to be waited for the promiseFn be resolved
     * @returns {Promise} which will eventually resolve to false when timeout (ms) has been reached
     */
     waitForTimeoutPromise: function( timeout ){
        return new Promise(( resolve, reject ) => {
