 ...
 *  - as an exception, logs from 'preCore' CLI main process are sent to the console if asked for.
 */
...
export class coreLogger {
    ...
    static logLevel( level ){
        if( level && typeof level === 'string' && level.length && Objects.values( coreLogger.l ).includes( level )){
            if( _logger ){
                ...
                _logger = pino( _options, pino.destination( _filename ));
            }
        }
    ...
    static logFile( fname ){
        if( fname && typeof fname === 'string' && fname.length ){
            if( _logger ){
                ...
                _logger = pino( _options, pino.destination( fname ));
                _filename = fname;
            }
        }
        return _filename;
     ...
     * @param {string} fname full pathanme to the log file
     * @returns {coreLogger}
     */
    constructor( opts, fname ){
        if( _logger ){
            return _logger;
        }
