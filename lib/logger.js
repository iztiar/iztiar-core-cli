...
function _log(){
    ...
    if( logger ){
        ...
        if( !logger.isLevelEnabled( _lower ) && process.env.IZTIAR_DEBUG && process.env.IZTIAR_DEBUG.includes( 'logLevel' )){
            console.log( fn( _emitter(), '(not allowed log level)', ...arguments ));
        //} else if( !Iztiar.envForked()){
        //    console.log( fn( _emitter(), ...arguments ));
        } else {
            logger[_lower]( _emitter(), arguments );
            //logger[_lower]( _emitter(), ...arguments );
            //logger[_lower]( _emitter(), Object.values( arguments ).joint( ' ' ));
        }
    // before any logger be instanciated, one can still log
    } else if( process.env.IZTIAR_DEBUG && process.env.IZTIAR_DEBUG.includes( 'preCore' )){
        console.log( fn( _emitter(), ...arguments ));
    }
}
...
export class coreLogger {
     ...
     * @param {string} new log level
     * @throws {coreResult}
     */
    static setLevel( fname ){
        if( logger ){
            utils.makeFnameDirExists( fname );
            logger = pino( options, pino.destination( fname ));
        }
    }
    ...
    /**
     * Instanciation should at least specify a prefix (the name of the owning process)
     * @constructor
    ...
    constructor( opts ){
        ...
        coreLogger.debug( 'instanciating new coreLogger()', opts );
        //console.log( opts );
        let _opts = {};
        let _prefix = null;
        if( opts ){
