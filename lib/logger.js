...
function _log( level ){
    ...
    if( logger ){
        ...
        //  hoping that the 'warn' level is ok
        if( !logger.isLevelEnabled( _lower )){
            coreLogger.warn( 'level \''+_lower+'\' not allowed to write to the current destination' );
        }
    } else {
        console.log( _emitter(), 'coreLogger::'+_upper+' ', ...arguments );
    }
}
...
export class coreLogger {
    ...
    static trace(){
        _log( coreLogger.l.TRACE, ...arguments );
    }
