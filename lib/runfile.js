...
export class coreRunfile {
    ...
    */
     ...
     */
    getFor( forkable, key ){
        //coreLogger.debug( 'coreFunfile:getFor() forkable='+forkable+' key='+key );
        let _json = utils.jsonReadFileSync( this._runfname );
        let _value = null;
        if( _json && _json[forkable] && _json[forkable][key] ){
            _value = _json[forkable][key];
        }
        //coreLogger.debug( 'coreFunfile:getFor() returning value='+_value );
        return _value;
    }
