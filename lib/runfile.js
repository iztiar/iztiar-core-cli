 ...
 */
function _getJsonByName( name ){
    return _getJsonByPath( coreRunfile.runFname( name ));
}

function _getJsonByPath( fname ){
...
export class coreRunfile {
    ...
    /**
     * @param {string} name the name of the controller service
     * @returns {JSON} the content of the runfile for the named coreController
     * @throws {coreError} if name is empty, null or undefined
     */
    static getTopController( fname ){
        const _json = _getJsonByPath( fname ) || {};
        return _json[Iztiar.c.forkable.CONTROLLER] || {};
    }
