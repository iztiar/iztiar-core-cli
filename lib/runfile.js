...
export class coreRunfile {
    ...
    /**
     * @param {JSON} json the content of the runfile
     * @returns {Object|string} an object of found services as { forkable: { name, pid, port }, ... }, or an error message
     */
    static processesChecked( json ){
        let _processes = null;
        try {
            ...
            switch( e.message ){
                case coreError.e.RUNFILE_EMPTYCONTENT:
                    return 'invalid runfile (empty content)';
                case coreError.e.RUNFILE_NAMEUNSET:
                    return 'invalid runfile (missing name)';
                case coreError.e.RUNFILE_PIDUNSET:
                    return 'invalid runfile (missing pid)';
                case coreError.e.RUNFILE_PORTUNSET:
                    return 'invalid runfile (missing port)';
                default:
                    return e.message;
            }
        }
