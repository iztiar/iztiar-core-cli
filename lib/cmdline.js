 ...
 * 
 * Note:
 *  Verbose level is incremental: the option may be specified several times on the command-line and
 *  each occurrence will increment the final level.
 * 
 * Note:
 *  This is a design decision to display copyright message at the very startup of the program.
 *  The verbosity level has no action of that.
 */
import { Command, Option } from 'commander';

import { Iztiar, coreConfig, coreError, coreLogger, corePackage, msg, utils } from './imports.js';

const subs = [
    { name: 'install', description: '(to be done) install the named controller service' },
    { name: 'uninstall', description: '(to be done) uninstall the named controller service' },
    ...
    //{ name: 'list-plugins', description: 'list installed plugins' },
    { name: 'list-runnings', description: 'display the list of running controller services' },
    { name: 'list-tree', description: 'display the hierarchy tree of running controller services' }
];
...
function _parseVerbosity( value, previous ){
    return ( typeof value !== 'undefined' && utils.isInt( value )) ? value : previous + 1;
}
...
export class coreCmdline {
    ...
    /**
     * Write a first line in log file
     */
    static startupLog(){
        msg.info( 'program invoked with args', process.argv );
        msg.info( 'command-line arguments successfully parsed: identified action \''+coreCmdline.getAction()+'\'');
        msg.debug( 'coreCmdline.startupLog() options', coreCmdline.options());
    }
}
