...
function _initCommand(){
    ...
    _command = new Command()
        ...
        .option( '-f|--force-stop', 'force the stop (and the cleanup of the run files )', coreConfig.getDefaultForceStop())
...
export class coreCmdline {
     ...
     */
    static startupLog(){
        coreLogger.info( 'program invoked with args', process.argv );
        coreLogger.info( 'command-line arguments successfully parsed: identified action \''+coreCmdline.getAction()+'\'');
        coreLogger.debug( 'coreCmdline.startupLog() options', coreCmdline.options());
    }
}
