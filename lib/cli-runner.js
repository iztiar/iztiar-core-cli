...
Iztiar.envForked( process.env[Iztiar.c.forkable.uuid] );
if( !Iztiar.envForked()){
...
coreConfig.load();
const config = coreConfig.getControllerRuntimeConfig( coreCmdline.getOptions().name );

// initialize our logger
const log = new coreLogger( Iztiar.c.app.name, coreConfig.getLogFilename());

// check the environment
// check running Node.js against the required version from package.json
if( !Iztiar.envForked()){
    if( corePackage.isRunningNodeAcceptable()){
...
const exitCli = function( code ){
    ...
    if( forker && !Iztiar.envForked()){
        coreLogger.debug( 'exitCli() ipcTarget='+forker.ipcTarget+' ipcCount='+forker.ipcCount );
...
switch( coreCmdline.getAction()){
    ...
    case 'uninstall':
        break;
    case 'start':
        forker = cliStart( config, exitCli );
        break;
    case 'stop':
        const res = cliStop( config );
        break;
    case 'restart':
        forker = cliRestart( config, exitCli );
        break;
    case 'status':
        cliStatus();
        break;
