...
export function cliStatus( serviceName ){
    ...
    const _serviceStatusPromise = function( result, name ){
        ...
        if( result.reasons.length  ){
            console.log( chalk.yellow( '\''+name+'\' service exhibits', result.reasons.length, 'error message(s)' ));
            console.log( chalk.yellow( ' You may want use --force-stop option to remove the falsy \''+name+'\' from your run directory' ));
    ...
    services.every(( o ) => {
        ...
        _promise = _promise.then(( res ) => { return _serviceStatusPromise( res, o.name )});
        return true;
    });
