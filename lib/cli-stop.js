...
import { Iztiar, coreCmdline, coreError, coreForkable, coreLogger, coreRunfile, msg, utils } from './imports.js';
            ...
            return new Promise(( resolve, reject ) => {
                ...
                coreForkable.checkServiceByName( name, false )
                    .then(( _local ) => {
                        ...
                        if( expected ){
                            if( _local.startable ){
                                msg.out( chalk.green( 'Service \''+serviceName+'\' is not running. Gracefully exiting.' ));
                            ...
                            if( _local.startable ){
                                msg.out( chalk.green( 'Service(s) \''+serviceName+'\' successfully stopped.' ));
                            } else {
        ...
        if( res.stoppable ){
            return new Promise(( resolve, reject ) => {
                ...
                if( !controller || !Object.keys( controller ).length ){
                    ...
                    if( port ){
                        res.requested += 1;
                        msg.out( '(main) requesting coreController on port '+port+' to stop...');
                        utils.tcpRequest( port, 'iz.stop' )
                            .then(( _local ) => {
                                res.servers = _local.servers;
                                res.servers.every(( s ) => {
                                    res.requested += 1;
                                    msg.out( '(coreController) requesting '+s.name+' on port '+s.port+' to stop...' );
                                    return true;
                                });
                            }, ( failure ) => {
   ...
   // resolves to true when there is no more process alive
    ...
    const _resultPromise = function( res, name ){
        return new Promise(( resolve, reject ) => {
            ...
            msg.debug( 'cliStop().resultPromise()', 'exitCode='+Iztiar.exitCode(), res, );
            resolve( res );
        });
    };
