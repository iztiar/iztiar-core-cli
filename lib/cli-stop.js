...
export function cliStop( serviceName ){
        ...
        return new Promise(( resolve, reject ) => {
            ...
            coreForkable.checkServiceByName( name, false )
                .then(( _local ) => {
                    ...
                    if( expected ){
                        if( _local.startable ){
                            console.log( 'Service(s) \''+serviceName+'\' is not running. Gracefully exiting.' );
                ...
                if( !controller || !Object.keys( controller ).length ){
                    ...
                    if( port ){
                        ...
                        utils.tcpRequest( port, 'iz.stop' )
                            ...
                            .catch(( e ) => { coreLogger.error( 'stopController()', e.name, e.message )});
                    } else {
                    ...
                    if( broker && Object.keys( broker ).length ){
                        ...
                        if( port ){
                            ...
                            utils.tcpRequest( port, 'iz.stop' )
                                ...
                                .catch(( e ) => { coreLogger.error( 'stopBroker()', e.name, e.message )});
                        } else {
    ...
    // resolves with the list of processes among those provided still present
    const _countProcesses = function ( pids ){
        return new Promise(( resolve, reject ) => {
            //console.log( '_countProcesses() calling ps pids=', pids );
            ps({ pid: pids })
                .then(( _local ) => {
                ...
                .catch(( e ) => {
                    coreLogger.error( '_countProcesses()', e.name, e.message )
                    resolve( [] );
                });
        })
    }
