...
import { Iztiar, coreConfig, coreForker, coreLogger, coreResult, coreRunfile } from './imports.js';
...
export class coreForkable {
    ...
    static requestAnswer( port, command, cb ){
        ...
        try {
            ...
            client.on( 'error', ( e ) => {
                coreLogger.error( 'coreForkable.requestAnswer() receives', e );
                cb( e, null );
            });
        } catch( e ){
            cb( e, null );
     ...
     * @returns {Object} the result to be answered to the client
     * @throws {coreResult}
     */
    execute( cmd, refs ){
        ...
        if( !cmd || typeof cmd !== 'string' || !cmd.length ){
            throw new coreResult( coreResult.e.COMMAND_UNSET );
        }
        const _words = cmd.split( ' ' );
        //coreLogger.debug( 'words', _words );
        if( !Object.keys( refs ).includes( _words[0] )){
            throw new coreResult( coreResult.e.COMMAND_UNKNOWN );
        }
        if( !refs[_words[0]] || !refs[_words[0]].fn || typeof refs[_words[0]].fn !== 'function' ){
            throw new coreResult( coreResult.e.COMMAND_UNDEFINED );
        }
        return refs[_words[0]].fn( this, _words );
    }
