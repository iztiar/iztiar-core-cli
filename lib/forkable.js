...
import { Iztiar, coreConfig, coreForker, coreLogger, coreResult, coreRunfile } from './imports.js';
    ...
    function _onIPCMessage( child, forker, messageData ){
        ...
        const forkable = messageKeys[0];
        if( messageData[forkable].event === 'startup' ){
            //_onStartupMessage( child, forker, messageData );
            coreLogger.info( forkable+' successfully started with pid '+messageData[forkable].pid );
...
export class coreForkable {
    ...
    static fork( forker ){
        ...
        child.on( 'message', ( messageData ) => {
            coreLogger.info( 'coreForkable::on.message %o', messageData );
