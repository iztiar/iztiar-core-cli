...
export class coreForkable {
     ...
     */
     static startupOnIPCMessage( child, messageData ){
        coreLogger.debug( 'coreForkable.startupOnIPCMessage()', messageData.name, Object.keys( messageData )[0], messageData.event );
    ...
    static startupOnIPCWriteRun( messageData ){
        if( messageData.event === 'startup' ){
            ...
            coreRunfile.set( messageData.name, _forkable, messageData[_forkable] );
        }
    }
