...
export class coreForkable {
    ...
    //  startup message
    static fork( id, cbExit, cbOnMessage, parent ){
        ...
        child.on( 'message', ( obj ) => {
            ...
            if( obj.event === 'startup' && obj.status === 'OK' ){
                ...
                if( cbOnMessage && typeof cbOnMessage === 'function' ){
                    cbOnMessage( parent, obj );
                }
    ...
    advertiseParent( port ){
        coreLogger.debug( process.env[coreForkable.id]+' advertising parent' );
        try {
            process.send({ coreForkable: process.env[coreForkable.id], event: 'startup', pid: process.pid, port: port, status: 'OK' });
    ...
    // a pseudo-virtual function, whiom we wish being overriden in coreController
    setChildPort(){
        console.log( 'in Forkable ', arguments );
    }
}
