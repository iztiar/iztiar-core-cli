...
export class coreController extends coreForkable {
    ...
    static OnMessage( controller, obj ){
    ...
    getControllerStatus( cb ){
        ...
        this._brokers.every(( b ) => {
            coreBroker.getBrokerStatus( b, ( st ) => {
                res.brokers[b.pid] = {
    ...
    onMessage( obj ){
        //console.log( 'coreController::onMessage() ', obj );
        this._brokers[obj.pid] = {
            ...this._brokers[obj.pid],
            ...obj
        };
        //console.log( 'onMessage ', this._brokers[obj.pid] );
    }

    // child here is the Node.js ChildProcess as returned when forking the broker
    registerBroker( child ){
        this._brokers[child.pid] = {
            ...this._brokers[child.pid],
            ...{ child: child }
        };
        //console.log( 'registerBroker ', this._brokers[child.pid] );
    }
    ...
    start(){
            ...
            c.on( 'data', ( data ) => {
                ...
                _strs.every(( s ) => {
                    if( s && s.length ){
                        switch( s ){
                            case 'iz.status':
                                this.getControllerStatus(( res ) => {
                                    c.write( JSON.stringify( res )+'\r\n' );
                                });
                                break;
                            default:
                                const o = { code: coreController.e.UNKNOWN_COMMAND, command: s };
        ...
        const _port = this.getConfig().controller.port;
        this._server
