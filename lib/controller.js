...
import { Iztiar, coreBroker, coreCmdline, coreConfig, coreError, coreForkable, coreLogger, corePackage, coreRunfile } from './imports.js';
...
export class coreController extends coreForkable {
    ...
    static startupComputeTargetsCount( name, sceConfig, level=0, count=0 ){
        coreLogger.debug( 'coreController.startupComputeTargetsCount()', 'name='+name, 'entering with level='+level, 'count='+count );
        if( level > 1 ){
            throw new coreError( coreError.e.CONTROLLER_RECURSION );
        }
        count += 1;                                         // this coreController
        coreLogger.debug( 'coreController.startupComputeTargetsCount()', 'name='+name, 'level='+level, 'coreController count='+count );
        count += sceConfig.broker.enabled ? 1 : 0;          // the coreBroker attached to this coreController
        coreLogger.debug( 'coreController.startupComputeTargetsCount()', 'name='+name, 'level='+level, 'attached coreBroker count='+count );
        if( level === 0 ){
            ...
            coreLogger.warn( 'coreController.startupComputeTargetsCount()', name+' coreController is configured to managed sub-services, will be ignored here' );
        }
        return count;
    }
    ...
    startupSubProcesses(){
        ...
        if( _sceConfig.broker.enabled ){
            console.log( ' - ('+_sceName+' coreController) requesting for coreBroker to start...' );
        ...
        coreLogger.debug( 'startupSubProcesses() managed', _sceConfig.managed );
        if( coreCmdline.options().manager ){
            if( _sceConfig.managed.lengh ){
                ...
                coreLogger.warn( 'coreController.startupSubProcesses()', _msg );
            ...
            _sceConfig.managed.every(( c ) => {
                ...
                coreLogger.debug( 'coreController.startupSubProcesses()', 'process.argv', process.argv, 'args', _args );
            ...
            c.on( 'data', ( data ) => {
                ...
                try {
                    const _ocmd = this.execute( _req, coreController.c, ( res ) => {
                        ...
                        coreLogger.info( 'server answers to \''+_req+'\'' );
                    });
                } catch( e ){
    ...
    startupTerminate( cb ){
        ...
        let _count = 1;               
        const _brokerPort = this.getServiceConfig().broker.controller.port;
        if( _brokerPort ){
            _count += 1;
            if( cb && typeof cb === 'function' ){
        ...
        if( _brokerPort ){
            ...
            utils.tcpRequest( _brokerPort, 'iz.stop' )
                .then(( res ) => {
