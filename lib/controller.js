...
export class coreController extends coreForkable {
    ...
    static startupComputeTargetsCount( name, sceConfig, level=0, count=0 ){
        ...
        if( level === 0 ){
            ...
            msg.warn( 'coreController.startupComputeTargetsCount()', name+' coreController is configured to managed sub-services, will be ignored here' );
        }
        return count;
    }
            ...
            c.on( 'data', ( data ) => {
                ...
                try {
                    const _ocmd = this.execute( _req, coreController.c, ( res ) => {
                        ...
                        msg.info( 'server answers to \''+_req+'\'' );
                    });
                } catch( e ){
                    msg.error( 'coreController.startupStart() command management', e.name, e.message );
                    c.end();
                }
            })
            .on( 'error', ( e ) => {
    ...
    startupTerminate( cb ){
        ...
        if( _brokerPort ){
            msg.debug( 'coreController.startupTerminate() broker.port='+_brokerPort );
