...
export class coreController extends coreForkable {
    ...
    startupStart(){
        ...
        this._tcpServer = net.createServer(( c ) => {
            //msg.debug( 'coreController::start() incoming connection' );
            //console.log( c );
            c.on( 'data', ( data ) => {
                //console.log( data );
                ...
                const _req = _strs[0];
                msg.info( 'server receives \''+_req+'\' request' );
                try {
                    const _ocmd = this.execute( _req, coreController.c, ( res ) => {
                        c.write( JSON.stringify( res )+'\r\n' );
                        msg.info( 'server answers to \''+_req+'\' with', res );
                        c.end();
                    });
                } catch( e ){
    ...
    startupTerminate( cb ){
        ...
        const _terminate_process = function( cb ){
            ...
            msg.info( 'coreController terminated', 'name='+_name,'code='+process.exitCode );
            process.exit();
        }
        ...
        let _count = 1;
        let _tasks = { servers: []};
        _tasks.servers.push({ name:_name, forkable:Iztiar.c.forkable.CONTROLLER, port:this._tcpPort, pid:process.pid });
        // plus the broker
        const _broker = coreRunfile.getBroker( _name ) || {};
        if( utils.isInt( _broker.port )){
            _count += 1;
            _tasks.servers.push({ name:_name, forkable:Iztiar.c.forkable.BROKER, port:_broker.port, pid:_broker.pid });
        }
        // if this controller managed others, then also will request them
        this._managedControllers.every(( n ) => {
            const _mc = coreRunfile.getController( n ) || {};
            if( utils.isInt( _mc.port )){
                _count += 1;
                _tasks.servers.push({ name:n, forkable:Iztiar.c.forkable.CONTROLLER, port:_mc.port, pid:_mc.pid });
                const _mb = coreRunfile.getBroker( n ) || {};
                if( utils.isInt( _mb.port )){
                    _count += 1;
                    _tasks.servers.push({ name:n, forkable:Iztiar.c.forkable.BROKER, port:_mb.port, pid:_mb.pid });
                }
            }
            return true;
        });
        msg.debug( 'coreController.startupTerminate()', 'tasks', _tasks );
        if( cb && typeof cb === 'function' ){
            msg.debug( 'coreController.startupTerminate()', 'sending tasks back to the requester' );
            cb( _tasks );
        }
        msg.debug( 'coreController.startupTerminate() about to terminate '+_count+' service(s)' );
        ...
        // ask the managed controllers to stop themselves
        _tasks.servers.every(( s ) => {
            if( s.forkable === Iztiar.c.forkable.CONTROLLER ){
                msg.debug( 'coreController.startupTerminate() managed controller port='+s.port );
                utils.tcpRequest( s.port, 'iz.stop' )
                    .then(( res ) => {
                        msg.debug( 'coreController.startupTerminate() '+s.name+' managed coreController answers ', res );
                        _count -= 1;
                        msg.debug( 'coreController.startupTerminate() '+_count+' server(s) left' );
                        if( !_count ){
                    ...
                    .catch(( e ) => {
                        msg.error( 'coreController.startupTerminate()', e.name, e.message );
                    });
            }
            return true;
        });
        ...
        if( _broker.port ){
            msg.debug( 'coreController.startupTerminate() broker.port='+_broker.port );
            utils.tcpRequest( _broker.port, 'iz.stop' )
                .then(( res ) => {
                    msg.debug( 'coreController.startupTerminate() coreBroker answers ', res );
                    _count -= 1;
                    msg.debug( 'coreController.startupTerminate() '+_count+' server(s) left' );
                    if( !_count ){
                        _terminate_process( cb );
