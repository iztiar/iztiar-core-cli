...
export class coreController extends coreForkable {
     ...
     */
     static checkJsonRun( fname, json ){
        let res = false;
        if( json ){
            let key = coreForkable.c.FORKABLE_CONTROLLER; 
            if( !json[key] ){
                key = coreForkable.c.FORKABLE_BROKER;
                if( !json[key] ){
                    key = null;
                }
            }
    ...
    terminate(){
        if( this._server ){
            ...
            this._server.close(() => {
                let code = 0;
                utils.jsonRemoveKeySync( coreController.getJsonPath( self.getConfig().controller.name ), coreForkable.c.FORKABLE_CONTROLLER );
