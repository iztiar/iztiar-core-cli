...
export class coreController extends coreForkable {
    ...
    /**
     * check whether a json run file is valid
     */
     static checkJsonRun( fname, json ){
        const res = ( json && json[coreForkable.c.FORKABLE_CONTROLLER].listening ) ? true : false;
        if( !res ){
    ...
    static getForkOptions(){
        return {
            type: coreForkable.c.FORKABLE_CONTROLLER,
            flowEnded: false,
            ready: false,
            cbExit: null,
            cbStartup: null,
    ...
    start(){
        ...
        this._server
            .listen( _port, '0.0.0.0', () => {
                this.advertiseParent( _port, this.getConfig().controller.name );
            })
            .on( 'error', ( e ) => {
                this.errorHandler( e );
            })
